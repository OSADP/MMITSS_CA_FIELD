...
{
  MsgEnum::map_enum_t::laneType type; 
  uint8_t  id;
  uint16_t width;      // in centimetres
  uint16_t attributes; // 16 bits
  uint8_t nodeNums;    // [0..MAXNODES]
  uint8_t conNums;    // [0..MAXCONS]
  Node_element_t node[MAXNODES];
  Conn_element_t conn[MAXCONS];
};

struct Approach_element_t
{
  MsgEnum::map_enum_t::approachType type;
  uint8_t laneNums; // [0..MAXLANES]
  Lane_element_t lane[MAXLANES];
};

struct Mapdata_element_t
{
  // DSRCmsgID_t msgID
  // MsgCount_t  msgCnt (mapVersion)
  uint8_t mapVersion;
  // struct MapData__intersections *intersections (one Intersection per map-data)
  // intersections::Intersection::IntersectionID_t   id
  uint32_t id;
  // intersections::Intersection::Position3D  *refPoint
  int32_t ref_latitude;
  int32_t ref_longitude;
  int32_t ref_elevation;  //centimetres
  // intersections::Intersection::approaches
  uint8_t approachNums; //[0..MAXAPPROACHES]
  Approach_element_t approach[MAXAPPROACHES];
};
...
struct SSM_element_t
{
  ...
  uint8_t   msgCnt;       // INTEGER (0..127)
  uint32_t  id;           // IntersectionID
  uint8_t   status;       // 0 - noPriority, 1 - earlyGreen, 2 - greenExtension
  uint16_t  ms;           // INTEGER (0..599), deciseconds in minute
  uint32_t  priorityCause;// vehicle TemporaryID that is under priority
  uint8_t   requestNums;  // (0..MAXPRIOREQUESTS)
  SSM_request_t request[MAXPRIOREQUESTS];
};

struct MSG_UNION_t 
{
  MsgEnum::msg_enum_t::MSGTYPE type;
  char msgName[10];
