...
class AsnJ2735Lib
{
  private:
    ...
    static const std::map<uint32_t,MsgEnum::phasestate_enum_t::vehicular> msPhaseState2ColorMap;  // signalLightState -> lightColor
    static const std::map<uint32_t,MsgEnum::phasestate_enum_t::pedestrian> msPedState2ColorMap;   // pedSignalState -> crossWalkingSign
    ...
    static std::map<uint32_t,MsgEnum::phasestate_enum_t::vehicular> createLightColorMap(void);
    static std::map<uint32_t,MsgEnum::phasestate_enum_t::pedestrian> createPedLightColorMap(void);
    MsgEnum::phasestate_enum_t::vehicular getPhaseState(const uint32_t singalLightState) const;
    MsgEnum::phasestate_enum_t::pedestrian getPedPhaseState(const uint32_t pedLightState) const;
  ...
  public:
    ...
    // get static map info
    size_t getIntersectionNums(void) const;
    uint8_t getMapVersion(const uint32_t intersectionId) const;
    ...
    uint32_t getLaneLength(const uint32_t intersectionId,const uint8_t laneId) const;
    ...
    bool decode_ssm_payload(const char* ptr,const size_t size,SSM_element_t* ps_art) const;
    ...
    ssize_t encode_ssm_payload(const SSM_element_t* ps_art,char* ptr,const size_t size,bool withHeader) const;
