...
uint16_t GeoUtils::point2D_t::direction2pt(const point2D_t& p) const
{
  double heading = DsrcConstants::rad2deg(atan2(static_cast<double>(p.x - x), static_cast<double>(p.y - y))); // [-180..180] degree, relative to true north
  if (heading < 0)
  {
    heading += 360.0; // [0..360]
  }
...
void GeoUtils::geoPoint2geoRefPoint(const GeoUtils::geoPoint_t& geoPoint, GeoUtils::geoRefPoint_t & geoRef)
{
  ...
  geoRef.elevation = DsrcConstants::unit2hecto<int32_t>(geoPoint.elevation);
}
...
void GeoUtils::geoRefPoint2geoPoint(const GeoUtils::geoRefPoint_t & geoRef, GeoUtils::geoPoint_t& geoPoint)
{
  ...
  geoPoint.elevation = DsrcConstants::hecto2unit<int32_t>(geoRef.elevation);
}
...
void GeoUtils::setEnuCoord(const GeoUtils::geoPoint_t& geoPoint, GeoUtils::enuCoord_t& enuCoord)
{
  ...
  enuCoord.transMatrix.dSinLat = sin(latitude_r);
  enuCoord.transMatrix.dCosLat = cos(latitude_r);
  enuCoord.transMatrix.dSinLong = sin(longitude_r);
  enuCoord.transMatrix.dCosLong= cos(longitude_r);  
...
void GeoUtils::lla2ecef(const GeoUtils::geoPoint_t& geoPoint,GeoUtils::point3D_t& ptECEF)
{
  ...
  double longitude_r = DsrcConstants::deg2rad(geoPoint.longitude);
  // Radius of Curvature
  double N = DsrcConstants::ellipsoid_a/sqrt(1.0-pow(DsrcConstants::ellipsoid_e*sin(latitude_r), 2));
  // ECEF coordinates
  ptECEF.x = (N + geoPoint.elevation) * cos(latitude_r) * cos(longitude_r);
  ptECEF.y = (N + geoPoint.elevation) * cos(latitude_r) * sin(longitude_r);
  ptECEF.z = (N * (1-pow(DsrcConstants::ellipsoid_e, 2)) + geoPoint.elevation) * sin(latitude_r);
}
...
{ 
  ...
  ptECEF.z = enuCoord.pointECEF.z + (enuCoord.transMatrix.dCosLat) * ptENU.y + (enuCoord.transMatrix.dSinLat) * ptENU.z;
}
...
{
  // initial
  double p = sqrt(pow(ptECEF.x,2)+pow(ptECEF.y,2));
  double elevation = 0;
  double latitude = atan( ptECEF.z / (p * (1.0 - pow(DsrcConstants::ellipsoid_e,2))));
  double prev_latitude = latitude;
  double N;
  
  for (int i=0; i<10; i++)
  {
    N = DsrcConstants::ellipsoid_a / sqrt(1.0 - (pow(DsrcConstants::ellipsoid_e,2) * pow(sin(prev_latitude),2)));
    elevation = p/cos(prev_latitude) - N ;
    latitude = atan( ptECEF.z / (p * (1.0 - pow(DsrcConstants::ellipsoid_e,2) * N / (N + elevation))));
...
{
  // distance between lat/lon points using haversine formula
  double dLat = DsrcConstants::deg2rad(p2.latitude - p1.latitude);
  double dLon = DsrcConstants::deg2rad(p2.longitude - p1.longitude);
  double a = sin( dLat/2.0 ) * sin( dLat/2.0 ) + sin( dLon/2.0 ) * sin( dLon/2.0 ) * cos( DsrcConstants::deg2rad(p1.latitude) ) * cos( DsrcConstants::deg2rad(p2.latitude) ); 
...
long GeoUtils::cross(const GeoUtils::point2D_t& O, const point2D_t& A, const point2D_t& B)
{
  return (long)(A.x - O.x) * (B.y - O.y) - (long)(A.y - O.y) * (B.x - O.x);
}
void GeoUtils::projectPt2Line(const GeoUtils::point2D_t& startPoint, const GeoUtils::point2D_t& endPoint, const GeoUtils::point2D_t& pt, GeoUtils::projection_t& proj2line)
{
  ...
  proj2line.d = (double)crossProduct(vb,va) / sqrt(d);
  proj2line.length = sqrt(d);
}

GeoUtils::polygon_enum_t::polygonType GeoUtils::convexcave(const std::vector<point2D_t>& p, const size_t size)
{
  // for a convex polygon all the cross products of adjacent edges must have the same sign
  // while concave polygon will change sign 
  if (size < 3)
    return (GeoUtils::polygon_enum_t::COLINEAR);
  ...
  for (size_t i = 0; i < size; i++)
  {
    ...
    if (flag == 3)
    {
      return (GeoUtils::polygon_enum_t::CONCAVE);
    }
  }
  if (flag != 0)
  {
    return (GeoUtils::polygon_enum_t::CONVEX);
  }
  else
  {
    return (GeoUtils::polygon_enum_t::COLINEAR);
...
std::vector<GeoUtils::point2D_t> GeoUtils::convexHull2D(const std::vector<GeoUtils::point2D_t>& P,const size_t size)
{
  std::vector<GeoUtils::point2D_t> ret;
  
  std::vector<GeoUtils::point2D_t> D(2*size+1);
  size_t bot = size - 2;
  size_t top = bot + 3; 
  D[bot] = D[top] = P[2]; 
  ...
  for (size_t i = 0; i <= (top-bot); i++)
  {
    ret.push_back(D[bot + i]);
  }
...
bool GeoUtils::isPointInsidePolygon(const std::vector<point2D_t>& pPolygon, const size_t size, const GeoUtils::point2D_t& waypoint)
{
  // for convex polygon only
  // cross product between the vector (way-point->vertex) and the vertex (vertex->next_vertex) must have the same sign
  // when moving counter-clockwise or clockwise along the edges
  int flag = 0;
  long d;
  size_t j;
  GeoUtils::vector2D_t v1,v2;
  ...
  {
    j = (i+1) % size;
    v1.set(waypoint,pPolygon[i]);
    v2.set(pPolygon[i],pPolygon[j]);
    d = crossProduct(v1,v2);
    if (d < 0)
    {
      flag |= 1;
