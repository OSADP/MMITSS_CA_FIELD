...
size_t NmapData::MapDataStruct::getIntersectionNums(void) const
{
  return mpIntersection.size();
}

uint8_t NmapData::MapDataStruct::getMapVersion(uint32_t intersectionId) const
{
  uint8_t ret = 0;
  int index = getIndexByIntersectionId(intersectionId);
  if (index >= 0)
    ret = mpIntersection[index].mapVersion;
  return ret;
}
...
uint32_t NmapData::MapDataStruct::getLaneLength(const uint32_t intersectionId,const uint8_t laneId) const
{
  uint32_t ret = 0;
  vector<uint8_t> inds = getIndexesByIds(intersectionId,laneId);
  if(!inds.empty())
  {
    size_t nodenums = mpIntersection[inds[0]].mpApproaches[inds[1]].mpLanes[inds[2]].mpNodes.size();
    ret = mpIntersection[inds[0]].mpApproaches[inds[1]].mpLanes[inds[2]].mpNodes[nodenums - 1].dTo1stNode;
  }
  return (ret);
}
...
bool NmapData::MapDataStruct::readNmap(const char* ptr)
{
  // open nmap file 
  ...
  while (std::getline(IS_NMAP,line))
  {
    if (!line.empty())
    {
      ...
      else if (line.find("MAP_Version") == 0)
      {
        ...
        iss.clear();
        pIntersection->mapVersion = static_cast<uint8_t>(iTmp);
      }
      ...
      else if (line.find("Intersection_attributes") == 0)
      {
        iss.str(line);
        iss >> std::skipws >> s >> s;
        pIntersection->attributes = bitset<8>(s);
        iss.clear();
      }
      else if (line.find("Reference_point") == 0)
      {
        ...
        iss.clear();
        geoPoint.elevation /= DsrcConstants::deca;  // read-in elevation in decimetres, convert to meter  
      ...
      else if (line.find("Approach_type") == 0)
      {
        ...
        iss >> std::skipws >> s >> iTmp;
        iss.clear();
        if (iTmp < (unsigned short)(MsgEnum::map_enum_t::APPROACH) || iTmp > (unsigned short)(MsgEnum::map_enum_t::CROSSWALK))
        {
          cerr << "Wrong Approach_type for intersection " << pIntersection->name;
          cerr << " apprach " << static_cast<int>(pIntersection->mpApproaches[approachCount].id) << endl;
          IS_NMAP.close();
          return false;
        }
        pIntersection->mpApproaches[approachCount].type = static_cast<MsgEnum::map_enum_t::approachType>(iTmp);
      }
      ...
      else if (line.find("No_lane") == 0)
      {
        ...
        if (laneNums > 0)
        {
          ...
          if (pIntersection->mpApproaches[approachCount].type == MsgEnum::map_enum_t::APPROACH)
          {
            pIntersection->ingressIndex.push_back(approachCount);
          }
          else if (pIntersection->mpApproaches[approachCount].type == MsgEnum::map_enum_t::EGRESS)
          {
            pIntersection->egressIndex.push_back(approachCount);
          }
          else if (pIntersection->mpApproaches[approachCount].type == MsgEnum::map_enum_t::CROSSWALK)
          {
      ...
      else if (line.find("Lane_type") == 0)
      {
        ...
        iss >> std::skipws >> s >> iTmp;
        iss.clear();
        if (iTmp < (unsigned short)(MsgEnum::map_enum_t::TRAFFICLANE) || iTmp > (unsigned short)(MsgEnum::map_enum_t::CROSSWALKLANE))
        {
          cerr << "Wrong Lane_type for intersection " << pIntersection->name;
          cerr << " apprach " << static_cast<int>(pIntersection->mpApproaches[approachCount].id);
          cerr << " lane " << static_cast<int>(pIntersection->mpApproaches[approachCount].mpLanes[laneCount].id) << endl;
          IS_NMAP.close();
          return false;
        }
        else
          pIntersection->mpApproaches[approachCount].mpLanes[laneCount].type = static_cast<MsgEnum::map_enum_t::laneType>(iTmp);    
      }
      else if (line.find("Lane_attributes") == 0)
      {
        ...
        iss >> std::skipws >> s >> s;
        iss.clear();
        pIntersection->mpApproaches[approachCount].mpLanes[laneCount].attributes = bitset<16>(s);
      }
      ...
      else if (line.find("Lane") == 0) 
      {
        ...
        if (iTmp > 8)
        {
          cerr << "Wrong control phase " << static_cast<int>(iTmp) << " for intersection " << pIntersection->name;
          cerr << " apprach " << static_cast<int>(pIntersection->mpApproaches[approachCount].id);
          cerr << " lane " << static_cast<int>(pIntersection->mpApproaches[approachCount].mpLanes[laneCount].id) << endl;
          IS_NMAP.close();
          return false;
        }
        pIntersection->mpApproaches[approachCount].mpLanes[laneCount].controlPhase = static_cast<uint8_t>(iTmp & 0xFF);       
      }   
      ...
      else if (line.find("No_Conn_lane") == 0)
      {
        ...
        for (int i = 0;i<connectionNums;i++)
        {
          ...
          {
            cerr << "Failed parsing connectTo " << s << " for intersection " << pIntersection->name;
            cerr << " apprach " << static_cast<int>(pIntersection->mpApproaches[approachCount].id);
            cerr << " lane " << static_cast<int>(pIntersection->mpApproaches[approachCount].mpLanes[laneCount].id) << endl;
            IS_NMAP.close();
            return false;
          }
          else if (iTmp > (unsigned short)(MsgEnum::map_enum_t::STRAIGHT))
          {
            cerr << "Wrong connectTo laneManeuver " << static_cast<int>(iTmp) << " for intersection " << pIntersection->name;
            cerr << " apprach " << static_cast<int>(pIntersection->mpApproaches[approachCount].id);
            cerr << " lane " << static_cast<int>(pIntersection->mpApproaches[approachCount].mpLanes[laneCount].id) << endl;
            IS_NMAP.close();
            return false;            
          ...
          pIntersection->mpApproaches[approachCount].mpLanes[laneCount].mpConnectTo[i].laneManeuver 
            = static_cast<MsgEnum::map_enum_t::maneuverType>(iTmp); 
        }
      }
  ...
  for (int i = 0;i<2;i++)
  {
    ...
    for (size_t egressIndex=0;egressIndex<mpIntersection[index].mpApproaches[egress].mpLanes.size();egressIndex++)
    {
      ...
      while(1)
      {
          ...
          for (size_t jj=0;jj<mpIntersection[intersectionIndex].mpApproaches[ingress].mpLanes[ii].mpConnectTo.size();jj++)
          {
            if (mpIntersection[intersectionIndex].mpApproaches[ingress].mpLanes[ii].mpConnectTo[jj].intersectionId 
              ...
              && mpIntersection[intersectionIndex].mpApproaches[ingress].mpLanes[ii].mpConnectTo[jj].laneManeuver 
                == MsgEnum::map_enum_t::STRAIGHTAHEAD
        ...
        connect2.laneManeuver = MsgEnum::map_enum_t::STRAIGHT;
...
{ 
  ...
  for (size_t i=0;i<mpIntersection.size();i++)
  {
    ...
    for (size_t j=0;j<mpIntersection[i].mpApproaches.size();j++)
    {
      for (size_t k=0;k<mpIntersection[i].mpApproaches[j].mpLanes.size();k++)
      {
        ...
        {
        case MsgEnum::map_enum_t::APPROACH:
          ...
          break;
        case MsgEnum::map_enum_t::EGRESS:
  ...
  for (size_t i=0;i<mpIntersection.size();i++)
  {
    for (size_t j=0;j<mpIntersection[i].mpApproaches.size();j++)
    {
      if (mpIntersection[i].mpApproaches[j].type == MsgEnum::map_enum_t::APPROACH)
      {
...
void NmapData::MapDataStruct::buildPolygons(void)
{
  ...
  for (size_t i = 0; i < mpIntersection.size();i++)
  {
    ...
    for (size_t j = 0; j < mpIntersection[i].mpApproaches.size(); j++)
    {
      ...
      if (mpIntersection[i].mpApproaches[j].type == MsgEnum::map_enum_t::CROSSWALK)
        continue;
      if (mpIntersection[i].mpApproaches[j].mpLanes.empty())
        continue;
      crosswalkIndx = j / 2;
    ...
    for (uint8_t phase = 1; phase < 9; phase++)
    {
      ...
      for (size_t j=0;j<mpIntersection[i].mpApproaches.size();j++)
      {
        for (size_t k=0;k<mpIntersection[i].mpApproaches[j].mpLanes.size();k++)
        {
          ...
          {
            if (mpIntersection[i].mpApproaches[j].mpLanes[k].type == MsgEnum::map_enum_t::CROSSWALKLANE)
            { 
  ...
  const uint8_t nodeIndex,const uint8_t approachType,const GeoUtils::point2D_t& ptENU,const GeoUtils::motion_t& motionState) const
{
  ...
  if (approachType == MsgEnum::map_enum_t::APPROACH)
  {
  ...
  const GeoUtils::motion_t& motionState,GeoUtils::vehicleTracking_t& vehicleTrackingState) const
{
  ...
  for (size_t i=0; i<mpIntersection[intersectionIndex].mpApproaches[approachIndex].mpLanes.size(); i++)
  {
    ...
    if (mpIntersection[intersectionIndex].mpApproaches[approachIndex].type == MsgEnum::map_enum_t::APPROACH)
    {
      startNodeIndex = static_cast<uint8_t>(mpIntersection[intersectionIndex].mpApproaches[approachIndex].mpLanes[i].mpNodes.size() -1);
      approachType = MsgEnum::map_enum_t::APPROACH;
    }
    else
    {
      startNodeIndex = 0;
      approachType = MsgEnum::map_enum_t::EGRESS;
    }
...
bool NmapData::MapDataStruct::locateVehicleInMap(const GeoUtils::connectedVehicle_t& cv,GeoUtils::vehicleTracking_t& cvTrackingState) const
{
  ...
  if (cv.vehicleTrackingState.intsectionTrackingState.vehicleIntersectionStatus == GeoUtils::vehicleInMap_enum_t::INSIDE_INTERSECTION_BOX)
  {
    ...
    if (!approachList.empty())
    {
      ...
      {
        // check EGRESS only
        if (mpIntersection[intersectionIndex].mpApproaches[approachList[j]].type == MsgEnum::map_enum_t::EGRESS
...
void NmapData::MapDataStruct::saveNmap(const char* ptr) const
{
  FILE* fp;
...
void NmapData::MapDataStruct::nmap_fprintf(FILE* fp) const
{
  ...
  for (size_t i=0;i<mpIntersection.size();i++)
  {
    ...
    fprintf(fp,"MAP_Version\t%u\n",mpIntersection[i].mapVersion);
    fprintf(fp,"IntersectionID\t%u\n",mpIntersection[i].id);
    fprintf(fp,"Intersection_attributes\t%s\n",mpIntersection[i].attributes.to_string().c_str());
    ...
    for (size_t j=0;j<mpIntersection[i].mpApproaches.size();j++)
    {
      ...
      for (size_t k=0;k<mpIntersection[i].mpApproaches[j].mpLanes.size();k++)
      {
        ...
        fprintf(fp,"Lane_attributes\t%s\n",mpIntersection[i].mpApproaches[j].mpLanes[k].attributes.to_string().c_str());
...
void NmapData::MapDataStruct::nmapEgressNoConnectTo_fprintf(FILE* fp) const
{
  ...
  for (size_t i=0;i<mpIntersection.size();i++)
  {
    for (size_t j = 0; j < mpIntersection[i].mpApproaches.size(); j++)
    {
      if (mpIntersection[i].mpApproaches[j].type != MsgEnum::map_enum_t::EGRESS)
        continue;
      for (size_t k = 0; k < mpIntersection[i].mpApproaches[j].mpLanes.size(); k++)
      {
        if (mpIntersection[i].mpApproaches[j].mpLanes[k].mpConnectTo.empty())
        {
          fprintf(fp,"MAP_Name %s MAP_Version %u %zu.%u.%zu\n",
            mpIntersection[i].rsuId.c_str(),mpIntersection[i].mapVersion,i+1,mpIntersection[i].mpApproaches[j].id,k+1);
        }
      }
    }
  }
...
void NmapData::MapDataStruct::nmapIntersectionPolygon_fprintf(FILE* fp) const
{
  ...
  {
    fprintf(fp,"--------------------------------------------------------------------------------\n");
    fprintf(fp,"Intersection %s MAP_Version %u mpPolygon type %u\n",
      mpIntersection[i].rsuId.c_str(),mpIntersection[i].mapVersion,mpIntersection[i].mpPolygonType);
