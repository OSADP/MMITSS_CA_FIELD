...
namespace GeoUtils
{
	...
	struct geoRefPoint_t
	{
		...
		int32_t elevation;  // in centimeters
	...
	{ // unit of centimeters
		...
		{
			return (static_cast<uint32_t>(std::round(std::sqrt(x  * x + y * y))));
		};
		uint32_t distance2pt(const GeoUtils::point2D_t& p) const
		{
			return (static_cast<uint32_t>(std::round(std::sqrt((x - p.x) * (x - p.x) + (y - p.y) * (y - p.y)))));
		};
	...
	std::vector<GeoUtils::point2D_t> convexHull2D(const std::vector<GeoUtils::point2D_t>& P);
