...
OBUconfig::configObu::configObu(const char* ptr1,const char* ptr2)
{
  ...
  mpDISPsendAddr = new socketUtils::socketAddress_t;
...
bool OBUconfig::configObu::readSocketConfigFile(const char* ptr1)
{
  ...
  while (std::getline(IS_F,line))
  {
    if (!line.empty())
    {
      ...
      else if (line.find("dispSendSocket") == 0)
      {
        iss.str(line);
        iss >> std::skipws >> s >> s >> mSocketConfig.s_dispSendSocket;
        iss.clear();
      }
    }
  }
  ...
  if (mSocketConfig.s_nmapFile.empty() 
    ...
    || mSocketConfig.s_dispSendSocket.empty())
    return false;
  else
    return true;
}
...
bool OBUconfig::configObu::setupSocketAddress(void)
{
  ...
  bool isDISPsendSucceed = socketUtils::setSocketAddress(mSocketConfig.s_dispSendSocket.c_str(),(*mpDISPsendAddr));
  
  if (!isWMElistenSucceed || !isWMEsendSucceed || !isDISPsendSucceed)
    return false;
  else
    return true;
}
...
socketUtils::socketAddress_t OBUconfig::configObu::getSocketAddr(const OBUconfig::socketHandleEnum_t::socketHandle type) const
{
  ...
  case OBUconfig::socketHandleEnum_t::WMELISTEN:
    p = *mpWMElistenAddr;
    break;
  case OBUconfig::socketHandleEnum_t::WMESEND:
    p = *mpWMEsendAddr;
    break;    
  default:
    p = *mpDISPsendAddr;
